                    ...
                    for (int i = 0; i < numVoices; i++) {
                        ...
                        if (i > 0 && numSlices > 0) {
                            ...
                            if (voices[i].sliceChangeTimer <= 0.0f) {
                                ...
                                int baseIndex = (int)(shNorm * (float)(numSlices - 1));
                                std::uniform_int_distribution<int> offsetDist(-2, 2);
                                int newSliceIndex = clamp(baseIndex + offsetDist(randomEngine), 0, numSlices - 1);

                                // Avoid selecting the same slice
                                int attempts = 0;
                                while (newSliceIndex == voices[i].sliceIndex && numSlices > 1 && attempts < 10) {
                                    newSliceIndex = clamp(baseIndex + offsetDist(randomEngine), 0, numSlices - 1);
                                    attempts++;
                                }
