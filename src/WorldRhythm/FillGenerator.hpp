...
#include "PatternGenerator.hpp"  // For MAX_PATTERN_LENGTH

namespace WorldRhythm {

// MetaModule: 固定陣列結構取代 std::vector 返回值
struct FillPattern {
    float velocities[MAX_PATTERN_LENGTH];
    int length;

    FillPattern(int len = 0) : length(std::min(len, MAX_PATTERN_LENGTH)) {
        for (int i = 0; i < MAX_PATTERN_LENGTH; i++) velocities[i] = 0.0f;
    }
...
public:
    ...
    // ========================================
    FillPattern generateRoll(int lengthSteps, float intensity) {
        FillPattern pattern(lengthSteps);
        std::uniform_real_distribution<float> dist(0.0f, 1.0f);
        ...
        for (int i = 0; i < lengthSteps; i++) {
            ...
            if (dist(rng) < density) {
                ...
                pattern.velocities[i] = std::clamp(vel, 0.3f, 1.0f);
            }
        }

        // Ensure last hit for resolution (strong)
        pattern.velocities[lengthSteps - 1] = std::min(1.0f, 0.9f + velVar(rng));
    ...
    // ========================================
    FillPattern generateRollAccelerando(int lengthSteps, float intensity) {
        FillPattern pattern(lengthSteps);
        ...
        for (int i = 0; i < lengthSteps; i++) {
            ...
            if (shouldHit) {
                pattern.velocities[i] = std::clamp(vel, 0.4f, 1.0f);
            }
        }

        // Strong resolution
        pattern.velocities[lengthSteps - 1] = std::clamp(0.95f + velVar(rng), 0.9f, 1.0f);
    ...
    // ========================================
    FillPattern generateRollTriplet(int lengthSteps, float intensity) {
        FillPattern pattern(lengthSteps);
        std::uniform_real_distribution<float> velVar(-0.08f, 0.08f);
        ...
        for (int i = 0; i < lengthSteps; i++) {
            ...
            if (shouldHit) {
                ...
                pattern.velocities[i] = std::clamp(vel, 0.35f, 1.0f);
            }
        }
    ...
    // ========================================
    FillPattern generateRollStutter(int lengthSteps, float intensity) {
        FillPattern pattern(lengthSteps);
        std::uniform_real_distribution<float> dist(0.0f, 1.0f);
        std::uniform_real_distribution<float> velVar(-0.1f, 0.1f);
        ...
        for (int i = 0; i < lengthSteps; i++) {
            ...
            if (i % 8 == 6 || i % 8 == 7) {
                if (progress > 0.3f && dist(rng) < intensity * 0.7f) {
                    ...
                    pattern.velocities[i] = std::clamp(vel, 0.35f, 0.9f);
                }
            }
            ...
            if (progress > 0.85f && i % 2 == 0) {
                ...
                pattern.velocities[i] = std::clamp(vel, 0.5f, 1.0f);
            }
        }

        pattern.velocities[lengthSteps - 1] = 1.0f;
    ...
    struct PitchedRollPattern {
        PitchedRollNote notes[MAX_PATTERN_LENGTH];
        int length;
        PitchedRollPattern(int len = 0) : length(std::min(len, MAX_PATTERN_LENGTH)) {
            for (int i = 0; i < MAX_PATTERN_LENGTH; i++) {
                notes[i].velocity = 0.0f;
                notes[i].pitchOffset = 0.0f;
            }
        }
    };

    PitchedRollPattern generateRollPitched(int lengthSteps, float intensity) {
        PitchedRollPattern pattern(lengthSteps);
        std::uniform_real_distribution<float> velVar(-0.08f, 0.08f);
        ...
        for (int i = 0; i < lengthSteps; i++) {
            ...
            if (shouldHit) {
                pattern.notes[i].velocity = std::clamp(
                    0.5f + progress * 0.45f * intensity + velVar(rng),
                    0.4f, 1.0f);
                // Pitch rises from 0 to 12 semitones
                pattern.notes[i].pitchOffset = progress * 12.0f * intensity;
            } else {
                pattern.notes[i].velocity = 0.0f;
                pattern.notes[i].pitchOffset = 0.0f;
            }
        }

        pattern.notes[lengthSteps - 1].velocity = 1.0f;
        pattern.notes[lengthSteps - 1].pitchOffset = 12.0f * intensity;
    ...
    // ========================================
    FillPattern generateTihai(int lengthSteps, float intensity) {
        ...
        FillPattern phrasePattern = generateTihaiPhrase(phraseLength, intensity);
        ...
        for (int rep = 0; rep < 3; rep++) {
            ...
            for (int i = 0; i < phraseLength && pos < lengthSteps; i++) {
                float vel = phrasePattern.velocities[i] * repScale;
                pattern.velocities[pos++] = std::clamp(vel + velVar(rng), 0.3f, 1.0f);
            }
        ...
        if (samPosition < lengthSteps) {
            pattern.velocities[samPosition] = std::clamp(1.0f + velVar(rng), 0.95f, 1.0f);
        }
    ...
    // ========================================
    FillPattern generateTihaiPhrase(int length, float intensity) {
        FillPattern phrase(length);
        ...
        for (int i = 0; i < length; i++) {
            ...
            phrase.velocities[i] = std::clamp(baseVel * intensity + velVar(rng), 0.4f, 1.0f);
        }
    ...
    // ========================================
    FillPattern generateBuildup(int lengthSteps, float intensity) {
        FillPattern pattern(lengthSteps);
        std::uniform_real_distribution<float> dist(0.0f, 1.0f);
        std::uniform_real_distribution<float> velVar(-0.1f, 0.1f);
    ...
    // ========================================
    FillPattern generateBreak(int lengthSteps) {
        FillPattern pattern(lengthSteps);

        // Only hit on first and last beat (strong accents)
        pattern.velocities[0] = 0.95f;
        if (lengthSteps > 1) {
            pattern.velocities[lengthSteps - 1] = 1.0f;  // Final hit strongest
        }
    ...
    struct AngselPatternData {
        ...
        AngselPatternData(int len = 0) : length(std::min(len, MAX_PATTERN_LENGTH)),
                                         silenceStart(0), silenceEnd(0), isUnison(true) {
            for (int i = 0; i < MAX_PATTERN_LENGTH; i++) velocities[i] = 0.0f;
        }
    };

    AngselPatternData generateAngsel(int lengthSteps, float intensity) {
        // 最小長度檢查：Angsel 需要至少 8 步才能形成有意義的三段結構
        // 信號區 2 步 + 靜默區 4 步 + 重入區 2 步 = 8 步
        static constexpr int ANGSEL_MIN_LENGTH = 8;

        AngselPatternData angsel(lengthSteps);
    ...
    // 簡化版本（只返回 FillPattern，供 generateFillPattern 使用）
    FillPattern generateAngselSimple(int lengthSteps, float intensity) {
        AngselPatternData angselData = generateAngsel(lengthSteps, intensity);
        FillPattern result(lengthSteps);
        for (int i = 0; i < lengthSteps; i++) {
            result.velocities[i] = angselData.velocities[i];
        }
        return result;
    }
    ...
    // ========================================
    FillPattern generateSignal(int lengthSteps, float intensity) {
        FillPattern pattern(lengthSteps);
        std::uniform_real_distribution<float> dist(0.0f, 1.0f);
        std::uniform_real_distribution<float> velVar(-0.1f, 0.1f);

        // Start with strong accent
        pattern.velocities[0] = std::clamp(0.9f + velVar(rng), 0.8f, 1.0f);
        ...
        for (int i = 1; i < lengthSteps - 1; i++) {
            ...
            if (dist(rng) < prob) {
                ...
                pattern.velocities[i] = std::clamp(vel, 0.3f, 0.85f);
            }
        }

        // End with accent
        pattern.velocities[lengthSteps - 1] = std::clamp(0.95f + velVar(rng), 0.85f, 1.0f);
    ...
    // ========================================
    FillPattern generateLlamada(int lengthSteps, float intensity) {
        FillPattern pattern(lengthSteps);
        std::uniform_real_distribution<float> velVar(-0.05f, 0.05f);
        ...
        if (lengthSteps < 4) {
            // 降級為簡單的開頭 + 結尾重音
            pattern.velocities[0] = std::clamp(0.95f * intensity + velVar(rng), 0.85f, 1.0f);
            if (lengthSteps > 1) {
                pattern.velocities[lengthSteps - 1] = std::clamp(1.0f + velVar(rng), 0.95f, 1.0f);
            }
            return pattern;
        }
        ...
        int clavePositions[8];
        int numClavePositions = 0;
        if (lengthSteps >= 16) {
            clavePositions[0] = 0; clavePositions[1] = 3; clavePositions[2] = 6;
            clavePositions[3] = 10; clavePositions[4] = 12;
            numClavePositions = 5;
        } else if (lengthSteps >= 8) {
            // 壓縮版本（8-15 steps）
            clavePositions[0] = 0; clavePositions[1] = 2; clavePositions[2] = 4;
            clavePositions[3] = 6; clavePositions[4] = 7;
            numClavePositions = 5;
        } else {
            // 簡化版本（4-7 steps）
            clavePositions[0] = 0; clavePositions[1] = 1; clavePositions[2] = 3;
            numClavePositions = 3;
        }
        ...
        for (int i = 1; i < lengthSteps - 1; i++) {
            ...
            for (int c = 0; c < numClavePositions; c++) {
                if (std::abs(mappedPos - clavePositions[c]) <= 1) {
                    isClavePos = true;
                    break;
                }
            }
            ...
            if (isClavePos) {
                ...
                pattern.velocities[i] = std::clamp(vel, 0.6f, 0.95f);
            }
        }
        ...
        int noteCount = 0;
        for (int i = 0; i < lengthSteps; i++) {
            if (pattern.velocities[i] > 0.0f) noteCount++;
        }

        // 如果太稀疏，補充音符
        if (noteCount < 3 && lengthSteps >= 8) {
            int midPoint = lengthSteps / 2;
            if (pattern.velocities[midPoint] < 0.01f) {
                pattern.velocities[midPoint] = std::clamp(0.75f * intensity + velVar(rng), 0.65f, 0.9f);
            }
        }
    ...
    // Returns FillPattern (velocities array + length)
    // ========================================
    FillPattern generateFillPattern(FillType type, int lengthSteps, float intensity) {
        switch (type) {
            case FillType::ROLL:
                return generateRoll(lengthSteps, intensity);
            case FillType::TIHAI:
            ...
            case FillType::ROLL_PITCHED: {
                ...
                PitchedRollPattern pitched = generateRollPitched(lengthSteps, intensity);
                FillPattern result(lengthSteps);
                for (int i = 0; i < lengthSteps; i++) {
                    result.velocities[i] = pitched.notes[i].velocity;
                }
                return result;
            }
            ...
            default:
                return FillPattern(lengthSteps);
        }
    }
