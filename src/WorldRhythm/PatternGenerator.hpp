...
// MetaModule: 固定陣列大小
static constexpr int MAX_PATTERN_LENGTH = 32;
...
struct Pattern {
    float velocities[MAX_PATTERN_LENGTH];  // 0.0 = no onset, 0.01-1.0 = velocity
    bool accents[MAX_PATTERN_LENGTH];
    int length;

    // v0.18.9: 確保 length 至少為 1，防止除零錯誤
    Pattern(int len = 16) : length(std::max(1, std::min(len, MAX_PATTERN_LENGTH))) {
        clear();
    }
...
public:
    PatternGenerator() : rng(12345) {}  // Fixed seed for MetaModule compatibility
    ...
    Pattern generate(Role role, const StyleProfile& style, int length, float density, float variation) {
        ...
        float weights[MAX_PATTERN_LENGTH] = {0.0f};
        ...
        if (density > 0.5f) {
            ...
            float expandedWeights[MAX_PATTERN_LENGTH];
            for (int i = 0; i < MAX_PATTERN_LENGTH; i++) expandedWeights[i] = weights[i];
            ...
            for (int i = 0; i < MAX_PATTERN_LENGTH; i++) weights[i] = expandedWeights[i];
        }
        ...
        weightedSelect(p, weights, length, targetOnsets);
    ...
    Pattern generateFoundationWithInterlock(const StyleProfile& style, int length,
        ...
        if (dist(rng) < beat3Prob) {
            skeleton[skeletonCount++] = beat3Pos;
        }
    ...
    void smoothVelocities(Pattern& p) {
        ...
        float smoothed[MAX_PATTERN_LENGTH] = {0.0f};
...
private:
    ...
    void weightedSelect(Pattern& p, float* weights, int length, int targetOnsets) {
