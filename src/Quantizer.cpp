...
struct QuantizerWidget : ModuleWidget {
    QuantizerWidget(Quantizer* module) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            addParam(createParamCentered<RoundSmallBlackKnob>(leftCoords[i], module, Quantizer::C_MICROTUNE_PARAM + noteIndex));
        }
        
        // Place white keys (right side) - removed labels
        for (int i = 0; i < 7; i++) {
            int noteIndex = rightPositions[i];
            addParam(createParamCentered<RoundSmallBlackKnob>(rightCoords[i], module, Quantizer::C_MICROTUNE_PARAM + noteIndex));
        }
